...
def record_fit(model, corpus=None, *, prevalence=None, prevalence_names=None,
    ...
    corpus : the ``Corpus`` (or token lists) the model was fit on. Optional: when
        ...
        which e.g. :class:`~topica.TreeFieldTM` exposes). A model that does not retain
    ...
    # Default the corpus to the one the model retained (e.g. TreeFieldTM.corpus), so record_fit(model)
