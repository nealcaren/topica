...
Two edge cases are surfaced rather than hidden. A debate with fewer than two speeches cannot be scaled and raises an error. And if the speaker-domain graph is *disconnected* — two blocs of speakers that never debate the same topics — the scale is not identified across the blocs; `num_components` reports it and `fit` warns, because the positions of speakers in different components are not comparable. We validate against an R reference oracle (`quanteda.textmodels` Wordfish per debate plus the paper's stage-2 ascent): on a two-stage synthetic corpus topica and the oracle recover the same actor scale at correlation 1.00, and both recover the planted positions (`parity/wordshoal_r_compare.py`).
