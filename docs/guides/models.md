...
`AnchorLDA` is validated against the [`anchor-topic`](https://pypi.org/project/anchor-topic/) reference implementation: the co-occurrence matrix is bit-identical, the full pipeline agrees to mean cosine 1.0 on a separable corpus, and topica reaches a tighter RecoverKL optimum than the reference on real text (`parity/anchor_compare.py`).
