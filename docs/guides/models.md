...
Anchor-words trades a little coherence for determinism and speed. On poliblog (K=15) it reaches about 93% of fully-converged LDA's c_v in roughly a fifth of the time, and it beats short-run LDA on both quality and time; on separable synthetic data it recovers the planted topics almost exactly. Its known weakness is inherent to the method, not the solver: neither recovery suppresses corpus-frequent words as aggressively as Gibbs does, because high-frequency terms genuinely co-occur with everything, so they can leak into several topics. It is a strong fast first pass and a deterministic baseline; for a final model on real text, compare against `LDA` or `STM`.
