...
**Initialization matters here more than in count-based LDA.** With per-token random initialization — what the reference's Gibbs sampler does — Gaussian LDA is prone to **mode collapse**: on well-separated data it will merge clusters and leave a topic empty, and this happens in both topica and the reference (every run of the original Java sampler on a clean three-cluster corpus drops one topic to empty). This is the instability the paper's k-means initialization exists to fix, so topica **defaults to `init="kmeans"`** (seeded k-means over the vocabulary embeddings), which recovers well-separated clusters reliably with no empty topics. Pass `init="random"` to reproduce the reference sampler's behavior exactly.

We validate in `parity/gaussian_lda_gold.py` against the authors' Apache-2.0 Java reference. The reference initializes from an unseeded RNG, so it is not itself reproducible run to run and the target is topic-aligned agreement, not bit-exactness: on a fixed-seed planted-cluster corpus, topica's per-topic means (fit with `init="random"` for a like-for-like comparison) align to the reference's at cosine 0.94, above the reference's own two-run floor, and the reference's `avgLL` trajectory is reproduced. The rank-1 Cholesky machinery is checked independently by a Rust unit test asserting the incrementally maintained per-topic mean and Cholesky factor equal the batch Normal-Inverse-Wishart sufficient statistics after a sequence of adds and removes, and the Student-t density matches a direct numpy computation to floating-point precision. On the shipped k-means default, topica recovers planted cluster centers at cosine 1.0 across seeds. Gaussian LDA is a dense Gibbs sampler with a per-topic `O(E^2)` density evaluation, so its cost grows with the embedding dimension; it is the right tool when you want topics that reason in embedding space, and ETM or the count-based models when you do not.
