...
tests = topica.diagnostics.word_intrusion(model, n_words=5, seed=0)
for t in tests:
...
```python
tests = topica.diagnostics.document_intrusion(model, texts=texts, n_docs=3, seed=0)
```
...
cv          = topica.diagnostics.coherence(model, texts, coherence_type="c_v", topn=10)
exclusivity = topica.diagnostics.exclusivity(model, 10)          # per topic

frontier = topica.diagnostics.quality_frontier(model, n=10)      # tidy: coherence, exclusivity, prevalence
```
...
```python
boot = topica.diagnostics.bootstrap_stability(docs, k=20, n_boot=50, iters=800)
...
pairs = topica.diagnostics.align_topics(a, b)                    # one-to-one matching (Hungarian)
print("stability across seeds:", topica.diagnostics.topic_stability([a, b], topn=10))
```
...
```python
labels = topica.interpret.label_topics(model.topic_word, model.vocabulary, n=8)  # prob & FREX
html = topica.interpret.find_thoughts_html(model, texts, n_docs=3, n_words=8)    # highlighted quotes
```
