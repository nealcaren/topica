...
```python
phrase_model = topica.prep.learn_phrases(docs, min_count=8, threshold=12.0)
docs = topica.prep.apply_phrases(docs, phrase_model)
```
...
`topica.prep.prep_documents` prunes a `Corpus` by document frequency — dropping
...
corpus_pruned, meta_pruned = topica.prep.prep_documents(
    corpus, meta=meta_df,
...
```python
topica.viz.plot_removed(corpus, thresholds=range(1, 15))
```
