"""Offline gold-fixture parity for topica ZeroShotTM vs a PyTorch AVITM reference (#271, Wave 1).

Loads the committed gold (``parity/zeroshottm_gold.npz`` + ``.json``), fits topica
ZeroShotTM on the SAME planted-block corpus + per-document embeddings frozen in the
gold (the encoder reads the embedding alone), Hungarian-aligns its topic-word matrix
to the reference's, and asserts the aligned cosine clears the reference's own
seed-to-seed self-cosine floor (minus a small margin). On this clean planted-block
design the two implementations agree at ~0.97, so the bar is cleared by a wide
margin.
...
import zeroshot_gold  # noqa: E402


def test_zeroshottm_gold_present():
    npz, js = harness.gold_paths("zeroshottm")
    assert npz.exists(), (
        f"missing {npz}; regenerate with `python parity/zeroshot_gold.py --regenerate` "
        "(needs torch)"
    )
    assert js.exists(), f"missing provenance log {js}"


def test_zeroshottm_gold_shape():
    ...
    arrays, meta = harness.load_gold("zeroshottm")
...
def test_zeroshottm_gold_is_non_vacuous():
    ...
    arrays, _ = harness.load_gold("zeroshottm")
    ...
    assert jac < 0.5, (
        f"shuffled ZeroShotTM top-word Jaccard {jac:.3f} should be well below 0.5; "
        "the gate is vacuous"
    )


def test_zeroshottm_matches_committed_gold():
    """Refit topica ZeroShotTM and compare to the frozen AVITM-reference gold."""
    r = zeroshot_gold.run(verbose=False)
    assert r["passes"], (
        f"topica ZeroShotTM topic-word cosine {r['cosine']:.4f} below bar "
