"""Offline planted-gold test for topica PA (issue #271, Wave 2).

PA has NO external reference implementation, so this is a PLANTED
self-consistency gold (see ``parity/pa_gold.py``). It loads the committed gold
(``parity/pa_gold.npz`` + ``.json``), refits topica on the same fixed-seed
...
import pa_gold  # noqa: E402

NAME = "pa"


def test_pa_gold_present():
    npz, js = harness.gold_paths(NAME)
    assert npz.exists(), (
        f"missing {npz}; regenerate with `python parity/pa_gold.py --regenerate`"
    )
    assert js.exists(), f"missing provenance log {js}"


def test_pa_matches_committed_gold():
    # Asserts determinism (refit-vs-gold cosine), planted recovery, and the
    # Wave 0 validity invariants (run inside wave2.run).
    r = pa_gold.run(verbose=False)
    assert r["passes"], (
        f"topica PA refit-vs-gold cosine {r['cosine']:.4f} below bar "
        f"{r['cosine_bar']:.2f} or recovery failed {r['recovery']}; details: {r}"
    )


def test_pa_gold_is_non_vacuous():
    ...
    assert cos < bar, (
        f"shuffled PA topic-word cosine {cos:.4f} should be below the bar "
    ...
    assert jacc < 0.5, (
        f"shuffled PA top-word jaccard {jacc:.4f} too high; gate is weak"
    )
