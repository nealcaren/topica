"""Offline planted-gold test for topica PT (issue #271, Wave 2).

PT has NO external reference implementation, so this is a PLANTED
self-consistency gold (see ``parity/pt_gold.py``). It loads the committed gold
(``parity/pt_gold.npz`` + ``.json``), refits topica on the same fixed-seed
...
import pt_gold  # noqa: E402

NAME = "pt"


def test_pt_gold_present():
    npz, js = harness.gold_paths(NAME)
    assert npz.exists(), (
        f"missing {npz}; regenerate with `python parity/pt_gold.py --regenerate`"
    )
    assert js.exists(), f"missing provenance log {js}"


def test_pt_matches_committed_gold():
    # Asserts determinism (refit-vs-gold cosine), planted recovery, and the
    # Wave 0 validity invariants (run inside wave2.run).
    r = pt_gold.run(verbose=False)
    assert r["passes"], (
        f"topica PT refit-vs-gold cosine {r['cosine']:.4f} below bar "
        f"{r['cosine_bar']:.2f} or recovery failed {r['recovery']}; details: {r}"
    )


def test_pt_gold_is_non_vacuous():
    ...
    assert cos < bar, (
        f"shuffled PT topic-word cosine {cos:.4f} should be below the bar "
    ...
    assert jacc < 0.5, (
        f"shuffled PT top-word jaccard {jacc:.4f} too high; gate is weak"
    )
