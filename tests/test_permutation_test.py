"""Tests for topica.effects.permutation_test (issue #36).
...
def test_returns_list_of_permutation_results():
    ...
    results = topica.effects.permutation_test(m, docs, cov, n_perm=5, seed=0, iters=50)
    assert isinstance(results, list)
    assert len(results) == 2
    for r in results:
        assert isinstance(r, topica.effects.PermutationResult)


def test_pvalue_in_unit_interval():
    m, docs, cov = _planted_corpus()
    results = topica.effects.permutation_test(m, docs, cov, n_perm=10, seed=0, iters=50)
    for r in results:
...
def test_null_shape():
    ...
    n_perm = 8
    results = topica.effects.permutation_test(m, docs, cov, n_perm=n_perm, seed=0, iters=50)
    for r in results:
...
def test_planted_topic_has_extreme_observed_effect():
    ...
    results = topica.effects.permutation_test(m, docs, cov, n_perm=20, seed=7, iters=100)
...
def test_planted_topic_pvalue_small():
    ...
    results = topica.effects.permutation_test(m, docs, cov, n_perm=30, seed=3, iters=100)
...
def test_random_covariate_pvalue_not_consistently_small():
    ...
    results = topica.effects.permutation_test(m, docs, random_cov, n_perm=20, seed=5, iters=50)
...
def test_topics_restriction():
    m, docs, cov = _planted_corpus()
    results = topica.effects.permutation_test(m, docs, cov, n_perm=5, seed=0,
                                      topics=[0], iters=50)
...
def test_raises_on_non_binary_covariate():
    ...
    with pytest.raises(ValueError, match="binary"):
        topica.effects.permutation_test(m, docs, bad_cov, n_perm=2, iters=20)


def test_raises_on_wrong_length_covariate():
    m, docs, _ = _planted_corpus(n=60)
    with pytest.raises(ValueError):
        topica.effects.permutation_test(m, docs, np.ones(10), n_perm=2, iters=20)
...
def test_as_dict_keys():
    ...
    results = topica.effects.permutation_test(m, docs, cov, n_perm=3, seed=0, iters=30)
    d = results[0].as_dict()
...
def test_corpus_object_accepted():
    ...
    # Should not raise.
    results = topica.effects.permutation_test(m, corpus, cov, n_perm=3, seed=0, iters=30)
    assert len(results) == 2
...
def test_covariate_model_permutation_runs():
    ...
    results = topica.effects.permutation_test(m, docs, cov, n_perm=4, seed=0, iters=50)
    assert len(results) == 2
    for r in results:
        assert isinstance(r, topica.effects.PermutationResult)
...
def test_covariate_threaded_into_refit(monkeypatch):
    ...
    n_perm = 3
    topica.effects.permutation_test(m, docs, cov, n_perm=n_perm, seed=0, iters=40)
...
def test_pvalue_never_exactly_zero():
    ...
    results = topica.effects.permutation_test(m, docs, cov, n_perm=20, seed=0, iters=80)
    for r in results:
        assert r.pvalue > 0.0, (
