"""Offline planted-gold test for topica EmbeddingLDA (issue #271, Wave 2).

EmbeddingLDA has NO external reference implementation, so this is a PLANTED
self-consistency gold (see ``parity/embeddinglda_gold.py``). It loads the committed gold
(``parity/embeddinglda_gold.npz`` + ``.json``), refits topica on the same fixed-seed
...
import embeddinglda_gold  # noqa: E402

NAME = "embeddinglda"


def test_embeddinglda_gold_present():
    npz, js = harness.gold_paths(NAME)
    assert npz.exists(), (
        f"missing {npz}; regenerate with `python parity/embeddinglda_gold.py --regenerate`"
    )
    assert js.exists(), f"missing provenance log {js}"


def test_embeddinglda_matches_committed_gold():
    # Asserts determinism (refit-vs-gold cosine), planted recovery, and the
    # Wave 0 validity invariants (run inside wave2.run).
    r = embeddinglda_gold.run(verbose=False)
    assert r["passes"], (
        f"topica EmbeddingLDA refit-vs-gold cosine {r['cosine']:.4f} below bar "
        f"{r['cosine_bar']:.2f} or recovery failed {r['recovery']}; details: {r}"
    )


def test_embeddinglda_gold_is_non_vacuous():
    ...
    assert cos < bar, (
        f"shuffled EmbeddingLDA topic-word cosine {cos:.4f} should be below the bar "
    ...
    assert jacc < 0.5, (
        f"shuffled EmbeddingLDA top-word jaccard {jacc:.4f} too high; gate is weak"
    )
