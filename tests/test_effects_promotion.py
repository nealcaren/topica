...
def test_aliases_point_to_one_implementation():
    assert topica.effects.estimate_effect is topica.effects.estimate_effect
    assert topica.effects.estimate_effect is topica.stm.estimate_effect
    assert topica.effects.by_strata is topica.effects.by_strata is topica.keyatm.by_strata
    assert topica.effects.top_topics is topica.keyatm.top_topics
    assert topica.effects.posterior_theta_samples is topica.stm.posterior_theta_samples
...
def test_dirichlet_theta_samples_shape_and_determinism():
    ...
    a = topica.effects.dirichlet_theta_samples(m.doc_topic, lengths, nsims=15, seed=0)
    b = topica.effects.dirichlet_theta_samples(m.doc_topic, lengths, nsims=15, seed=0)
    ...
    short = topica.effects.dirichlet_theta_samples(m.doc_topic, np.full(len(docs), 5.0), nsims=40, seed=0)
    long = topica.effects.dirichlet_theta_samples(m.doc_topic, np.full(len(docs), 500.0), nsims=40, seed=0)
...
def test_estimate_effect_on_gibbs_draws_and_point():
    ...
    lengths = np.array([len(d) for d in docs])
    draws = topica.effects.dirichlet_theta_samples(m.doc_topic, lengths, nsims=20, seed=0)

    moc = topica.effects.estimate_effect(draws, x, feature_names=["grp"])
    ols = topica.effects.estimate_effect(m.doc_topic, x, feature_names=["grp"])
...
def test_prevalence_ci_is_model_neutral():
    ...
    out = topica.effects.prevalence_ci(m, groups, ci=0.9)
    ...
    flipped = topica.effects.prevalence_ci(m, groups, labels=["odd", "even"])
...
def test_prevalence_ci_falls_back_without_retained_draws():
    ...
    out = topica.effects.prevalence_ci(m, groups, corpus=docs, nsims=15, seed=0)
    assert out["mean"].shape == (2, 2)
    with pytest.raises(ValueError, match="one label per document"):
        topica.effects.prevalence_ci(m, groups[:-1], corpus=docs, nsims=10)


def test_dirichlet_validation():
    with pytest.raises(ValueError):
        topica.effects.dirichlet_theta_samples(np.zeros((4, 3)), np.zeros(5))  # mismatched lengths
    with pytest.raises(ValueError):
        topica.effects.dirichlet_theta_samples(np.zeros(4), np.zeros(4))  # theta not 2-D
