"""Offline planted-gold test for topica ETM (issue #271, Wave 2).

ETM has NO external reference implementation, so this is a PLANTED
self-consistency gold (see ``parity/etm_gold.py``). It loads the committed gold
(``parity/etm_gold.npz`` + ``.json``), refits topica on the same fixed-seed
...
import etm_gold  # noqa: E402

NAME = "etm"


def test_etm_gold_present():
    npz, js = harness.gold_paths(NAME)
    assert npz.exists(), (
        f"missing {npz}; regenerate with `python parity/etm_gold.py --regenerate`"
    )
    assert js.exists(), f"missing provenance log {js}"


def test_etm_matches_committed_gold():
    # Asserts determinism (refit-vs-gold cosine), planted recovery, and the
    # Wave 0 validity invariants (run inside wave2.run).
    r = etm_gold.run(verbose=False)
    assert r["passes"], (
        f"topica ETM refit-vs-gold cosine {r['cosine']:.4f} below bar "
        f"{r['cosine_bar']:.2f} or recovery failed {r['recovery']}; details: {r}"
    )


def test_etm_gold_is_non_vacuous():
    ...
    assert cos < bar, (
        f"shuffled ETM topic-word cosine {cos:.4f} should be below the bar "
    ...
    assert jacc < 0.5, (
        f"shuffled ETM top-word jaccard {jacc:.4f} too high; gate is weak"
    )
