...
import gsdmm_gold  # noqa: E402

NAME = "gsdmm"


def test_gsdmm_gold_present():
    npz, js = harness.gold_paths(NAME)
    assert npz.exists(), (
        f"missing {npz}; regenerate with `python parity/gsdmm_gold.py --regenerate`"
    )
    assert js.exists(), f"missing provenance log {js}"


def test_gsdmm_matches_committed_gold():
    # Asserts determinism (refit-vs-gold cosine), planted recovery, and the
    # Wave 0 validity invariants (run inside wave2.run).
    r = gsdmm_gold.run(verbose=False)
    assert r["passes"], (
        f"topica GSDMM refit-vs-gold cosine {r['cosine']:.4f} below bar "
        f"{r['cosine_bar']:.2f} or recovery failed {r['recovery']}; details: {r}"
    )


def test_gsdmm_gold_is_non_vacuous():
    ...
    assert cos < bar, (
        f"shuffled GSDMM topic-word cosine {cos:.4f} should be below the bar "
    ...
    assert jacc < 0.5, (
        f"shuffled GSDMM top-word jaccard {jacc:.4f} too high; gate is weak"
    )
