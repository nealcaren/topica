"""Offline planted-gold test for topica HDP (issue #271, Wave 2).

HDP has NO external reference implementation, so this is a PLANTED
self-consistency gold (see ``parity/hdp_gold.py``). It loads the committed gold
(``parity/hdp_gold.npz`` + ``.json``), refits topica on the same fixed-seed
...
import hdp_gold  # noqa: E402

NAME = "hdp"


def test_hdp_gold_present():
    npz, js = harness.gold_paths(NAME)
    assert npz.exists(), (
        f"missing {npz}; regenerate with `python parity/hdp_gold.py --regenerate`"
    )
    assert js.exists(), f"missing provenance log {js}"


def test_hdp_matches_committed_gold():
    # Asserts determinism (refit-vs-gold cosine), planted recovery, and the
    # Wave 0 validity invariants (run inside wave2.run).
    r = hdp_gold.run(verbose=False)
    assert r["passes"], (
        f"topica HDP refit-vs-gold cosine {r['cosine']:.4f} below bar "
        f"{r['cosine_bar']:.2f} or recovery failed {r['recovery']}; details: {r}"
    )


def test_hdp_gold_is_non_vacuous():
    ...
    assert cos < bar, (
        f"shuffled HDP topic-word cosine {cos:.4f} should be below the bar "
    ...
    assert jacc < 0.5, (
        f"shuffled HDP top-word jaccard {jacc:.4f} too high; gate is weak"
    )
