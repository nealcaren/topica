"""Offline planted-gold test for topica ECTM (issue #271, Wave 2).

ECTM has NO external reference implementation, so this is a PLANTED
self-consistency gold (see ``parity/ectm_gold.py``). It loads the committed gold
(``parity/ectm_gold.npz`` + ``.json``), refits topica on the same fixed-seed
...
import ectm_gold  # noqa: E402

NAME = "ectm"


def test_ectm_gold_present():
    npz, js = harness.gold_paths(NAME)
    assert npz.exists(), (
        f"missing {npz}; regenerate with `python parity/ectm_gold.py --regenerate`"
    )
    assert js.exists(), f"missing provenance log {js}"


def test_ectm_matches_committed_gold():
    # Asserts determinism (refit-vs-gold cosine), planted recovery, and the
    # Wave 0 validity invariants (run inside wave2.run).
    r = ectm_gold.run(verbose=False)
    assert r["passes"], (
        f"topica ECTM refit-vs-gold cosine {r['cosine']:.4f} below bar "
        f"{r['cosine_bar']:.2f} or recovery failed {r['recovery']}; details: {r}"
    )


def test_ectm_gold_is_non_vacuous():
    ...
    assert cos < bar, (
        f"shuffled ECTM topic-word cosine {cos:.4f} should be below the bar "
    ...
    assert jacc < 0.5, (
        f"shuffled ECTM top-word jaccard {jacc:.4f} too high; gate is weak"
    )
