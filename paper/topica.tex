...
\code{STM}. The toolkit is built to be a \emph{disciplined} successor to \pkg{LDAvis}
...
and near-duplicate topics, and refuses panels a model cannot support: a
...
fairest yardstick for stochastic inference. We report it without inflating it. The
...
them we report wall-clock \emph{to convergence}, the time a user actually waits,
...
topic distinctness. The comparison costs no extra plumbing because every
...
produced with \pkg{topica} 0.32.0 by \code{paper/reproduce.py}; the exact
