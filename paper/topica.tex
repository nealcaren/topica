...
\proglang{R} \pkg{stm}'s at a topic-word cosine of $0.976$ (per-topic median
...
the published topics at a topic-word cosine of $0.91$, against $0.08$ for a
...
threshold: $0.91$ sits just below the $0.97$ at which \pkg{topica}'s already
...
\code{STS} agrees with its own \code{STM} at $0.92$, confirming that \code{STS}
...
produced with \pkg{topica} 0.28.0 by \code{paper/reproduce.py}; the exact
