...
Section~\ref{sec:validation} summarizes the cross-implementation agreement as a
single alignment cosine per model. This appendix shows the underlying picture: for
...
settings, and their output is displayed twice, side by side. Each subsection aligns
the two engines' topics (by topic-word cosine, except keyATM's keyword topics,
which are index-aligned by construction) and reports the top words from each, plus
the feature that distinguishes that model: covariate prevalence effects (STM, DMR),
topic correlations (CTM), per-group wording (content/SAGE), seeded keywords
(keyATM), the super/sub-topic hierarchy (PA), the inferred topic count (HDP), and
covariate-driven sentiment (STS). LDA is shown against two independent reference
implementations at once---Java \pkg{MALLET} and \pkg{tomotopy}---since both are
collapsed-Gibbs samplers that agree with \pkg{topica}'s. For the spectral-initialized
structural models (STM, CTM, content) the two engines start from the same
deterministic anchor words and the topics coincide almost exactly; for the
collapsed-Gibbs models (LDA, DMR, HDP, PA) each engine initializes at random, so
as Section~\ref{sec:validation} notes the agreement is statistical---the dominant
topics coincide while the diffuse, low-prevalence tail topics, which differ
between any two random-initialized runs, account for most of the alignment gap
(for the nonparametric HDP the headline result is instead that both engines infer
nearly the same topic count without being told it).
...
CLI, \pkg{tomotopy}, or the \pkg{STS} replication package) was absent at build time
are marked skipped.

\input{generated/validation_appendix}
