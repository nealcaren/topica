...
%% --- JSS submission source. The local JSS style files and logo come from the
%% official template at https://www.jstatsoft.org/style. make_arxiv.sh changes
%% this to the masthead-free nojss variant in its temporary arXiv build.
...
diagnostics to support those decisions, and matches the uncertainty it reports to
what each model can support. A parallel \proglang{Rust} core
makes the fits fast and reproducible. Fit to convergence, the structural and other
variational models run roughly one and a half to three times faster than
\proglang{R} \pkg{stm} on a single core and five to seven times faster on a typical
multicore workstation (faster still per iteration, depending on the machine and
corpus), and \pkg{topica} is at parity with the compiled \pkg{MALLET} and
\pkg{keyATM} samplers, while reproducing the variational models to the bit and the
samplers from a fixed seed.
Each model on the default surface is validated against its reference
implementation where one exists, or by recovery and invariance checks where the
model is novel. We describe the design, the
...
apply to every model and support cross-model comparison
...
implementation, and we report the agreement as measured, including where two correct
...
versions checked in mid-2026 (\pkg{MALLET} 2.0.8, \pkg{gensim} 4.4.0,
\pkg{tomotopy} 0.13.0, \proglang{R} \pkg{stm} 1.3.8, \pkg{keyATM} 0.5.5,
\pkg{BERTopic} 0.17.3). ``STM/content'' is a full structural topic model with
...
The contract is one of shared attribute \emph{names and shapes}, not always of an
identical statistical object. For the count-based models \code{doc\_topic} is a
proper distribution whose rows lie on the probability simplex; for the
matrix-factorization models (\code{NMF}, \code{LSA}) the same attribute holds
signed latent coordinates that do not sum to one, and the embedding-cluster
models expose a hard or soft assignment rather than a generative posterior. The
diagnostics that assume a simplex (for example held-out perplexity) are therefore
defined only for the generative models and refuse, rather than silently
mis-compute, for the others; the conformance suite records exactly which surface
each model supports.
...
Reproducibility is designed into the implementation. The correlated and
structural topic models produce bit-for-bit identical fits on repeated runs,
...
the threads happen to finish. The dynamic topic model and the collapsed-Gibbs
models are reproducible from a fixed seed and thread count. The practical
consequence is that refitting under different seeds provides a direct stability
check: with the corpus and all other settings held fixed, the seed is the only
source of variation.
...
In every case \pkg{topica} \emph{implements} a method introduced elsewhere; the
statistical contribution belongs to the cited authors, and \pkg{topica} is an
implementation and workflow integration, not a reimagining of the underlying
models. The Reference column of Table~\ref{tab:models} gives the originating
paper for each model, and a user who fits one should cite that paper alongside
\pkg{topica} \citep{caren2026topica}. This is most consequential for the methods
whose own software social scientists already rely on---the structural topic model
\citep{roberts2016model} and keyword-assisted topic modeling
\citep{eshima2024keyatm}---where \pkg{topica} reimplements the published model
family and validates against the authors' package (Section~\ref{sec:validation})
rather than supplanting it.

\begin{table}[t!]
\centering
\caption{The \pkg{topica} model family. Nearly all present the \code{fit}\,$\rightarrow$\,
\code{topic\_word}/\code{doc\_topic} contract of Section~\ref{sec:design}; \code{HLDA}
(a topic hierarchy) and \code{DTM} (time-sliced topics) are structural exceptions
with no single $(D, K)$ \code{doc\_topic}.}
\label{tab:models}
\small
...
\subsection{Measure effects with defensible uncertainty}
...
The same principle reaches the parameters a model estimates directly, not only
the downstream effect regression: where a model reports a point estimate,
\pkg{topica} attaches an uncertainty to it. The covariate weights of \code{DMR}
and \code{GDMR} and of the covariate \code{KeyATM} carry a \code{feature\_effect\_se}
from the observed information of the penalized Dirichlet-multinomial they
maximize; the ideal-point family reports a \code{position\_se} for each estimated
position; \code{SupervisedLDA} reports a standard error on each regression
coefficient and a posterior-predictive standard deviation on each prediction; and
the \code{STM}/\code{CTM} topic correlations come with a credible interval. These
are analytic wherever the model affords one---the curvature of the fitted
objective or the variational posterior---so they are exact given the fit and
computed once, with the bootstrap reserved for the one quantity, topic coherence,
that has no likelihood or posterior to read an interval from. The discipline is
the same throughout: report the uncertainty the estimate actually carries, and
where there is none to report, say so.
...
\code{STM}. The toolkit extends \pkg{LDAvis} \citep{sievert2014ldavis} with
confidence bands where a posterior exists, explicit labels for topic-word
weights, a closure-corrected topic correlation, and indicators for \emph{dead}
...
\caption{The one-call report \code{topica.plot\_report(model, texts=texts, groups=groups)} for
...
We assess reimplementations by comparison with the reference implementations
used in social-science research. The repository includes scripts that run the
reference packages and report their outputs. The checks in \code{parity/} drive Java \pkg{MALLET} and
...
benchmarked against each reference's own run-to-run reproducibility. The
supplementary material shows the underlying picture in full: for fifteen models it
fits both the reference package and \pkg{topica} on the same poliblog corpus and
displays the recovered topics twice, side by side, word for word rather than as a
single summary statistic.

``Validated'' means different things for different models, and we are explicit
about which applies. We distinguish four kinds of evidence. \emph{Reference
parity} compares \pkg{topica} to an established implementation on shared inputs
(LDA against \pkg{MALLET}; STM, CTM, and content against \proglang{R}~\pkg{stm};
\code{KeyATM} against \proglang{R}~\pkg{keyATM}; NMF and LSA against
\pkg{scikit-learn}; and the side-by-side legs of the supplement). \emph{Planted
recovery} checks that a model recovers known structure on a synthetic corpus with
a fixed, separable answer---the right test where two stochastic engines need not
produce identical matrices, and the only one for a model with no single canonical
reference. \emph{Structural and invariance checks} verify properties the method
must have regardless of data: bit-for-bit determinism under a fixed seed, a
monotone variational bound, and conformance to the shared interface contract. The
neural and embedding models are validated against their own reference
implementations on the corpora those references use. Each of these applies to a
model on the \emph{default surface}; a fourth category, \emph{experimental} models
behind an explicit opt-in gate (Section~\ref{sec:extending}), is deliberately
excluded from the default surface precisely because it has not yet cleared a
reference-parity or recovery bar, and we make no validation claim for it. When we
say every model is validated, we mean every model a user gets by default.
...
\proglang{R} \pkg{stm}'s at a topic-word cosine of $0.975$ (per-topic median
$0.994$), far inside \proglang{R}'s own spectral-versus-random spread of about
...
\paragraph{Sentiment-discourse model.} \code{STS} is validated against the
...
We benchmark fit time (construction and import excluded) the way each model family
terminates. The variational structural models have a convergence criterion, so for
them we report wall-clock \emph{to convergence},
with both engines stopped at the same relative-bound tolerance
(\code{emtol}$=10^{-5}$); the collapsed-Gibbs models have no convergence criterion,
so there the user fixes a sweep budget and we report cost at a matched budget. The
structural timings are on a documented HPC node (a two-socket \proglang{AMD}
\pkg{EPYC} 7702 with OpenBLAS, \proglang{R} 4.5.0 / \pkg{stm} 1.3.8) on real
corpora; \proglang{R} \pkg{stm} is single-threaded by design, and we cap
\pkg{topica} at sixteen cores, a typical workstation rather than the full node. The
collapsed-Gibbs and thread-scaling comparisons (and Figures~\ref{fig:scaling} and
\ref{fig:threads}) are on a quiet 14-core Apple M-series machine, where the
memory-bound parallel samplers are free of the cross-tenant bandwidth contention an
HPC node's other jobs would impose.
Speed depends on corpus size, vocabulary, the number of topics, and hardware, so
the numbers are orders of magnitude, not guarantees. Each comparison is reproduced
by a script in \code{benchmarks/} that times the identical fits in \pkg{topica} and
the reference: \code{bench\_stm\_convergence.py} for the structural-model
time-to-convergence (Table~\ref{tab:stm}), \code{bench\_stm.py} for its
per-iteration decomposition, \code{bench.py} for the LDA, keyword-assisted, memory,
and thread-scaling figures, and \code{k\_crossover.py} for the \pkg{tomotopy}
topic-count crossover.

\begin{table}[t!]
\centering
\caption{\pkg{topica} versus \proglang{R} \pkg{stm}, wall-clock \emph{to
convergence} (both at \code{emtol}$=10^{-5}$, spectral start, $K=20$,
\code{$\sim$rating $+$ s(time)}) on a documented two-socket \proglang{AMD}
\pkg{EPYC} 7702 node, single-core and capped at sixteen cores; mean of two
repeats, with single-core speedups ranging $2.6$--$2.9\times$ (\code{poliblog})
and $1.6$--$1.8\times$ (Congress) across repeats. EM iterations to converge in
parentheses. \proglang{R} \pkg{stm} is single-threaded by design. At a fixed
tolerance \pkg{topica} drives its bound to a tighter point than \proglang{R} (more
iterations; see text), so its per-iteration speed exceeds these wall-clock ratios.}
\label{tab:stm}
\small
\begin{tabular}{lrrrrrr}
\toprule
& & \multicolumn{2}{c}{Single core} & \multicolumn{2}{c}{16 cores} & \\
\cmidrule(lr){3-4}\cmidrule(lr){5-6}
Corpus & docs & \pkg{topica} & \proglang{R} \pkg{stm} & \pkg{topica} & speedup & 1-core sp. \\
\midrule
\code{poliblog} & 5{,}000  & 23.8s (27) & 64.9s (20) & 9.3s (27)  & 7.0$\times$ & 2.7$\times$ \\
Congress        & 25{,}000 & 385s (81)  & 641s (49)  & 134s (81)  & 4.8$\times$ & 1.7$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
take minutes. Run to convergence (Table~\ref{tab:stm}), \pkg{topica} fits it
$1.7$ to $2.7$ times faster than \proglang{R} \pkg{stm} on a single core and
roughly five to seven times faster on sixteen cores, in about a quarter of
\proglang{R} \pkg{stm}'s memory (about 180MB against 675MB at 5{,}000 documents,
since it holds the corpus and the variational state in compact native arrays
rather than \proglang{R} objects). The per-iteration decomposition is sharper and
explains the wall-clock: \pkg{topica}'s variational E-step is roughly three to four
times cheaper per iteration single-threaded (eight to nine times on sixteen cores).
It runs \emph{more} EM iterations than \proglang{R}, though ($27$ versus $20$ on
\code{poliblog}, $81$ versus $49$ on Congress), because at a fixed \code{emtol} it
drives its evidence bound to a tighter optimum: past \proglang{R}'s stopping point
\pkg{topica}'s topic-word matrix still shifts, but by well under one percent
(aligned cosine above $0.99$ to its own converged fit). The to-convergence ratios therefore
understate \pkg{topica}'s engine speed, and the single-core advantage narrows on the
larger corpus, where those extra convergence iterations accumulate; the multicore
advantage, which is what a user on a workstation sees, holds near five times even there.
The full machine-stamped report is regenerated by \code{paper/reproduce.py} and
committed at \code{paper/generated/replication\_report\_longleaf.md} (the
machine-independent parity checks of Section~\ref{sec:validation} are in the
companion \code{replication\_report.md}).
To check that these structural results are not an artifact of a single machine, we
repeated the to-convergence benchmark on a second, architecturally distinct node (a
two-socket \proglang{Intel} \pkg{Xeon} Gold 6140). The EM iteration counts came out
identical to the \pkg{EPYC} run ($27$ versus $20$ on \code{poliblog}, $81$ versus
$49$ on Congress), as they should, since iterations to a fixed tolerance are a
property of the algorithm rather than the hardware. The single-core speedups land in
the same range ($1.7\times$ on Congress, $3.0\times$ on \code{poliblog}); only the
multicore multiple shifts with the node (about seven and eleven times respectively at
sixteen cores), as memory-bandwidth-bound parallel scaling does.
The keyword-assisted model matches \proglang{R} \pkg{keyATM}'s
\proglang{C++} sampler single-threaded (22.5s versus 23.0s on a
2{,}000-document, 10-topic fit of 1{,}000 sweeps) and runs about 1.7 times as fast
on four cores and 2.1 times on eight (13.6s and 11.0s), through a
document-partitioned parallel sweep that the \proglang{R} package has no
equivalent of.

Those figures are at $K=20$. The Laplace E-step stores a $(K-1)\times(K-1)$
posterior covariance per document, an $O(NK^2)$ term that dominates both memory
and time as the topic count grows, so \pkg{topica} gives two controls for the
large-$K$ regime (Figure~\ref{fig:scaling}). Storing that covariance as
\code{float32} halves it by default, and \code{keep\_eta\_cov=False} drops it
entirely, recomputing each document's covariance on demand only when
method-of-composition standard errors are requested; on the same
5{,}000-document corpus at $K=200$ this holds peak memory near 0.4GB against about
3.7GB for the stored full covariance, and roughly flat in $K$. Independently,
\code{variational="diagonal"} replaces the full Laplace covariance with a
mean-field diagonal, skipping the per-document $O(K^3)$ inverse: the same $K=200$
fit drops from about 31s to 17s, at the cost of the off-diagonal posterior
correlations. The default path is unchanged and bit-for-bit identical to prior
releases.

\begin{figure}[t!]
\centering
\includegraphics[width=0.85\textwidth]{fig_scaling}
\caption{Topic-count scaling of the logistic-normal fit on a 5{,}000-document
synthetic corpus (10 EM iterations). Left: peak resident memory with the default
stored covariance grows as $O(K^2)$, while \code{keep\_eta\_cov=False} recomputes
it on demand and stays roughly flat in $K$. Right: fit time for the default
Laplace E-step against the mean-field \code{variational="diagonal"} mode, which
skips the per-document $O(K^3)$ inverse. Reproduced by
\code{benchmarks/bench\_scaling.py}.}
\label{fig:scaling}
\end{figure}

The LDA comparison does not uniformly favor \pkg{topica}. Against \pkg{MALLET},
the original \proglang{Java} sampler, the two are at parity
...
5{,}000-document fit on eight threads runs in 13.0s against \pkg{MALLET}'s 21.2s),
...
trades speed for higher topic coherence and exact reproducibility. \code{"cvb0"}
extends to every count-based model that shares the collapsed conditional: the
per-document prior (\code{DMR}), seeded (\code{SeededLDA}), supervised
(\code{LabeledLDA}), and keyword (\code{KeyATM}) variants; \code{"warp"} covers the
plain-Dirichlet members (\code{LDA}, \code{DMR}, \code{SeededLDA}). CVB0 is the natural
...
This example uses a single corpus to illustrate comparison across models and
inference within one model. The shared contract supports a common comparison of
models, while the structural topic model supports covariate-effect estimation
with uncertainty. Both analyses use the political-blog corpus of the \pkg{stm}
...
Because every model exposes the same surface, a heterogeneous set can be scored
with a common loop. Here four models from two families, three
...
Coherence is windowed $c_v$ \citep{lau2014coherence}; exclusivity (stm's
FREX-sum over each topic's top words, roughly $0$--$10$) and diversity measure
topic distinctness. Every model presents the same analysis surface. Numbers
reproduce from \code{paper/replication.py}.}
\label{tab:spanning}
\small
...
single-model tool can pose. The embedding clustering of Table~\ref{tab:spanning}
has no posterior over $\theta$ to draw from, so its effect uncertainty comes from
the bootstrap rather than the method of composition. Across the family, then,
every effect carries an interval matched to what the model supports: propagated
from a posterior where one exists, and from resampling where it does not.
...
($+0.045$, 95\% CI $[0.034, 0.056]$), media coverage and bias ($+0.043$,
$[0.027, 0.058]$), and Israel ($+0.055$, $[0.043, 0.066]$); liberal blogs
disproportionately discuss McCain ($-0.084$, $[-0.094, -0.075]$) and the Bush
...
estimating $\theta$, not only sampling variation in the regression. The script
that produced this figure is \code{paper/replication.py}; the corresponding worked
example in the package documentation carries the analysis further, to selecting
$K$ and to the clustered-by-blog standard errors that nested data demand.
...
Section~\ref{sec:workflow} became available for the new model through the shared
interface.
...
of its algorithm rather than with surrounding infrastructure.
...
of $K$, the interpretation and labeling of topics, and the specification of
covariates. The package supplies diagnostics and uncertainty estimates to inform
those decisions. This distinction is especially relevant when large-language-model
agents are used in analysis pipelines; the package includes a companion guide for
such use.
...
\pkg{topica} is released under the Apache-2.0 license, which is compatible with
