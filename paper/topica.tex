...
\proglang{R} \pkg{stm} on a single core and five to eight times faster on a typical
...
\pkg{EPYC} 7713 with OpenBLAS, \proglang{R} 4.5.0 / \pkg{stm} 1.3.8) on real
...
\code{$\sim$rating $+$ s(time)}) on a two-socket \proglang{AMD} \pkg{EPYC} 7713,
single-core and capped at sixteen cores. EM iterations to converge in parentheses.
\proglang{R} \pkg{stm} is single-threaded by design. At a fixed tolerance
\pkg{topica} drives its bound to a tighter point than \proglang{R} (more
...
\midrule
\code{poliblog} & 5{,}000  & 24.7s (27) & 71.7s (20) & 8.9s (27)  & 8.0$\times$ & 2.9$\times$ \\
Congress        & 25{,}000 & 392s (81)  & 607s (49)  & 112s (81)  & 5.4$\times$ & 1.5$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
$1.5$ to $2.9$ times faster than \proglang{R} \pkg{stm} on a single core and $5.4$
to $8.0$ times faster on sixteen cores, in well under half \proglang{R} \pkg{stm}'s
memory (it holds the corpus and the variational state in compact native arrays
rather than \proglang{R} objects). The per-iteration decomposition is sharper and
explains the wall-clock: \pkg{topica}'s variational E-step is $2.6$ to $3.9$ times
cheaper per iteration single-threaded ($9$ to $11$ times on sixteen cores). It runs
\emph{more} EM iterations than \proglang{R}, though ($27$ versus $20$ on
...
advantage, which is what a user on a workstation sees, holds at $5$ to $8$ times.
