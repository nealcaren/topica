...
models it runs roughly ten to eighteen times faster than \proglang{R} \pkg{stm}
on a single core (depending on the machine and corpus) and up to thirty-two times
faster multithreaded, and it is at parity with
...
numbers are orders of magnitude, not guarantees. Each comparison here is
reproduced by a script in \code{benchmarks/} that times the identical fits in
\pkg{topica} and the reference: \code{bench\_stm.py} for the structural-model
timings (Table~\ref{tab:stm}), \code{bench.py} for the LDA, keyword-assisted,
memory, and thread-scaling figures, and \code{k\_crossover.py} for the
\pkg{tomotopy} topic-count crossover.

\begin{table}[t!]
\centering
\caption{\pkg{topica} versus \proglang{R} \pkg{stm}, fit time on matched EM
iterations from a spectral start, on an Apple M3 Max. \proglang{R} \pkg{stm} is
single-threaded by design; \pkg{topica} parallelizes the variational E-step. An
independent, thrice-repeated run on a Xeon-class HPC node reproduces the
single-core advantage on real data (reported below).}
\label{tab:stm}
\small
...
\proglang{R} objects.
An independent run on a documented HPC node (a two-socket \proglang{AMD}
\pkg{EPYC} 9654 with OpenBLAS, \proglang{R} 4.5.0 / \pkg{stm} 1.3.8), repeated
three times, reproduces this on real data: across 2{,}000 to 5{,}000
\code{poliblog} documents \pkg{topica} fits the same structural model fourteen to
eighteen times faster single-threaded (for example 1.8s against 32.6s at 2{,}000
documents) in roughly a quarter of \proglang{R} \pkg{stm}'s memory (180MB against
670MB). The full machine-stamped report is regenerated by \code{paper/reproduce.py}
and committed at \code{paper/generated/replication\_report\_longleaf.md} (the
