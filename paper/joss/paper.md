...
[@terragni2021octis] and TopMost [@wu2024topmost] collect classical and neural
models together with evaluation metrics and, in OCTIS, hyperparameter optimization.
None of these spans the classical, structural, keyword, and embedding families that
applied work draws on together, and none estimates how a topic varies with a
document attribute along with the uncertainty that claim requires, so a study that
wants to compare a structural model against a clustering of embeddings, and to read
how each topic moves with time, author, or treatment, must move data between
