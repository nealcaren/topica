...
- [**mechanistic-topic-models**](https://github.com/blei-lab/mechanistic-topic-models) (Zheng, Beltran-Velez, Karlekar, Shi, Nazaret, Mallik, Feder & Blei, 2025, [arXiv:2507.23220](https://arxiv.org/abs/2507.23220), MIT) — `MechanisticLDA`: the Mechanistic Topic Model (mLDA), LDA over sparse-autoencoder features instead of words. topica models a supplied document × feature count matrix (`topica.from_feature_matrix`); the SAE feature-extraction pipeline stays outside the core, following the "you bring the features" pattern of the embedding models. The topic model over features reuses the validated SparseLDA sampler (the fit is bit-identical to `LDA` on the equivalent bag-of-words corpus); it is experimental pending an end-to-end parity check against the reference's Gemma-2-9b pipeline.
