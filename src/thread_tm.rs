//! TreeFieldTM — a reply-threaded topic model.
//!
//! TreeFieldTM is CTM's logistic-normal topic model with **one structural change**: a document's
...
//! correlated logistic-normal prior CTM/STM fit). Roots get it so with no reply tree TreeFieldTM reduces
...
/// Fit TreeFieldTM by variational EM. `docs` are token-id lists, `parents[d]` the index of `d`'s
...
/// User supervision for TreeFieldTM (issue #854): steer the topic-word distributions and/or
...
pub fn fit_thread_tm<R: Rng, F: FnMut(usize, usize, f64) -> bool>(
    ...
    // (`crate::spectral`), so TreeFieldTM's logistic-normal base starts from an STM-quality point rather
    ...
    // root, so this makes TreeFieldTM's base equivalent to CTM. Init to the identity (≡ the old p0 = 1).
...
/// `root_siginv` (= Σ_root⁻¹), and the per-group `anchor` all fixed. This is `transform` for TreeFieldTM.
///
...
mod tests {
    ...
    /// prevalence field; fit TreeFieldTM; recover the topics and a non-degenerate κ.
    #[test]
