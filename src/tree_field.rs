...
//!
//! This is the one new numerical core of `TreeFieldTM` — a reply-threaded topic model that couples a
...
//!
//! `TreeFieldTM`'s M-step uses [`fit_fixed_mean`] here to estimate `(κ, σ²)` on anchor-centered
...
/// shared across dimensions and supplied by the caller (in TreeFieldTM it comes from the STM/CTM
...
/// Like [`fit`] but with the anchor mean `m` HELD at 0 — for TreeFieldTM, whose field is fit on
